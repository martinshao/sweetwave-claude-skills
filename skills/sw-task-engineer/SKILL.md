...
3. 每个任务包含稳定 ID、任务类型、角色、项目、风险、QA、并行、依赖、范围、AC 和验证命令。
...
6. 更新已有任务时保留 `[x]`；使用 `[NEW]`、`[CHANGED]`、`[DROPPED]`
   表达变更，不重写已完成任务。
7. `app-shell` 只生成 `APP-SHELL-001` scaffold 任务；其他前端页面任务使用
   `app-shell/APP-SHELL-001` 作为依赖。
8. 不实现代码，不修改 PLAN_STATE、STATUS 或 RUN_STATE。
